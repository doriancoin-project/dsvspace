  ...
  test('should return defaults when no file is present', () => {
    jest.isolateModules(() => {
      ...
      expect(config.MEMPOOL).toStrictEqual({
        ...
        POOLS_JSON_TREE_URL: 'https://api.github.com/repos/doriancoin-project/mining-pools-dsv/git/trees/master',
        POOLS_JSON_URL: 'https://raw.githubusercontent.com/doriancoin-project/mining-pools-dsv/master/pools.json',
        AUDIT: false,
      ...
      expect(config.EXTERNAL_DATA_SERVER).toStrictEqual({
        ...
        MEMPOOL_ONION: 'http://TODO: doriancoin.onion/api/v1',
      });
